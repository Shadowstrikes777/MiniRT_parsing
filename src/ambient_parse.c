...
/*   Updated: 2025/04/04 18:18:04 by mmaevani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ambient_parse(char *line, t_data *data)
{
	...
	cursor = 2;
	// in case of non unique return -2
	if (data->ambient.unique == true
...
static int	parse_red(char *line, t_data *data, int *cursor)
{
	...
	if (data->ambient.rgb[0] < 0 || data->ambient.rgb[0] > 255)
		return (-1);
	*cursor += ft_strchr(line, ',') - line + 1; // will point to the character after the comma
	return (0);
}
...
static int	parse_blue(char *line, t_data *data, int *cursor)
{
	...
	if (data->ambient.rgb[2] < 0 || data->ambient.rgb[2] > 255)
		return (-1);
	*cursor += ft_strchr(line, '\n') - line + 1; // will point to the character after the comma
	return (0);
}
