...
/*   Updated: 2025/07/14 16:40:58 by mihrakot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (render.y >= 0)
	{
		...
		while (render.x < WIDTH)
		{
			...
			my_mlx_pixel_put(&render.img, render.x, HEIGHT - 1 - render.y,
				create_rgb(render.ray_col.x,
					render.ray_col.y, render.ray_col.z));
			render.x++;
		}
		render.y--;
	}
